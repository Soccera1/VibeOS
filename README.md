...
- **VFS:** Read-only initramfs (`cpio newc`) root with an `ext2`/`ext3` or read-only XFS `/usr` mount path, a writable `/home` XFS mount or ramdisk fallback, a writable volatile `/tmp` ramdisk, plus support for pipes, symlinks, Unix-domain sockets, and device nodes (`/dev/tty`, `/dev/null`, `/dev/fb0`). The shipped `/usr` and `/home` images are XFS.
...
make gconfig         # Edit configuration in GTK 3, falling back to Libadwaita, GTK 4, then GTK 2
make aconfig         # Edit configuration in GTK 4/Libadwaita
...
y/n to set booleans, ? for help, s to save, and q to quit. Value editing supports
Backspace, Ctrl-U to clear, Enter to accept, and Escape to cancel. Unavailable
options are marked with `-`. Saving writes `.config` and both generated files;
quitting with changes offers save, discard, or cancel. Terminal size defaults to
80x24 when unavailable. `make check-tconfig` runs pseudo-terminal regression tests.
...
GTK 3's C API when available, then falls back to GTK 4/Libadwaita, GTK 4, then GTK 2.
...
development libraries (`libgtk-4-dev` on Debian/Ubuntu). `make aconfig` builds
and runs the Libadwaita frontend, requiring `pkg-config` and GTK 4/Libadwaita
development libraries (`libadwaita-1-dev` on Debian/Ubuntu). `xconfig` retains
...
`fltk-config`. `mconfig` uses the Motif C API and requires Motif, Xt, and
...
`make check-guiconfig` exercises the selected GTK frontend, Libadwaita, GTK 4, Qt, FLTK, Motif, and Tk on a desktop
display or Xvfb, `make check-g2config` tests GTK 2 explicitly,
`make check-aconfig` tests Libadwaita alone, `make check-g4config` tests GTK 4 alone, `make check-fconfig` tests FLTK alone,
...
| Filesystems | `EXT2` | Reject ext2/ext3 mounts and disable ext2 writes. `/usr` and `/home` automount remain available when XFS is enabled. |
| Filesystems | `XFS` | Reject XFS mounts. XFS support is independent of ext2/ext3. |
| Filesystems | `XFS_WRITE` | Reject writable XFS mounts; the default `/home` falls back to a ramdisk when enabled. |
| Filesystems | `EXT2_WRITE` | Reject writable ext2/ext3 mounts with `EROFS`, while allowing read-only mounts. The default XFS `/home` is unaffected. |
...
The initramfs now carries the root filesystem, the essential `/bin` command set, BusyBox, and empty `/usr`, `/home`, and `/tmp` mountpoints. The kernel filesystem backends support ext2/ext3, read-only XFS v4/v5, and writable XFS v5 images. On the default GPT/QEMU run path, `build/usr.xfs` is attached as virtio SCSI target 0 and mounted read-only at `/usr`, while `build/home.xfs` is attached as target 1 and mounted read-write at `/home`; `/tmp` is always backed by a volatile writable ramdisk. Both GPT and ISO GRUB configurations also load `/usr` as a Multiboot module; the kernel prefers that module when present. The kernel can also mount ext2 or ext3 filesystems from regular files through the existing ext2/ext3 loopback path when those files are already reachable through VFS, but there is not yet a kernel block-device/boot-filesystem reader for opening `/boot/usr.xfs` directly from the boot medium.
...
disk or Multiboot module. `tools/make_home_xfs.sh` creates a writable 384 MiB
`build/home.xfs` using the supported v5 feature profile. The GPT boot partition
remains ext3. Existing `build/home.ext3` images are left untouched; the new image
starts empty and does not automatically migrate their contents.
...
inode numbers, and v5 metadata CRC32c verification. Writable mounts support
allocation, file and directory mutations, links, metadata updates, and native
buffer journal recovery, enabled by `CONFIG_KERNEL_XFS_WRITE`. They require
the restricted v5 feature profile described in
[the filesystem manual](docs/chapters/filesystems.texi). Unsupported feature
sets and foreign dirty log formats are rejected. Read-only mounts do not
replay logs and require clean images. Run `make check-xfs-kernel` to test the
default home image builder with static musl and dynamic glibc programs across
writes and reboot.
