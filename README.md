...
- **VFS:** Read-only initramfs (`cpio newc`) root with a read-only `ext3` `/usr` mount, plus support for pipes, symlinks, and device nodes (`/dev/tty`, `/dev/null`).
...
qemu-system-x86_64 \
  ...
  -drive format=raw,file=build/home.ext3,if=ide,index=1 \
  -serial stdio
```
...
The initramfs now carries the root filesystem, the essential `/bin` command set, BusyBox, and empty `/usr` and `/home` mountpoints. A separate `build/usr.ext3` image is loaded as a second Multiboot module and mounted read-only at `/usr`, where the non-essential GNU utilities and optional standalone programs are exposed under `/usr/bin`. A separate `build/home.ext3` image is attached as a second IDE disk and mounted read-write at `/home` for persistent mutable state. The kernel can also mount an ext3 filesystem from a regular file through the existing ext2/ext3 loopback path when that file is already reachable through VFS, but the shipped boot images still rely on the Multiboot module path for `/usr` because there is not yet a kernel block-device/boot-filesystem reader for opening that file directly from the boot medium.
