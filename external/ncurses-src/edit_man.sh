...
(*.[0-9]*)
	...
	if test ! -f "/home/lily/code/vibeos/external/ncurses-src/man_alias.sed" ; then
cat >>/home/lily/code/vibeos/external/ncurses-src/man_alias.sed <<-CF_EOF2
		s,@DATADIR@,$datadir,g
...
s,@TSET@,tset,g
CF_EOF2
		echo "...made /home/lily/code/vibeos/external/ncurses-src/man_alias.sed"
	fi
	...
	aliases=`sed -f "$top_srcdir/man/manlinks.sed" "$cf_full_alias" |sed -f "/home/lily/code/vibeos/external/ncurses-src/man_alias.sed" | sort -u; test "$cf_part_alias" = "$nCurses" && echo curses`
	...
	sed	-f "/home/lily/code/vibeos/external/ncurses-src/man_alias.sed" \
		< "$i" >$TMP
	...
	if test "$verb" = installing ; then
		...
		test -n "$aliases" && (
			cd "$cf_subdir${section}" && (
				...
				test -n "bz2" && cf_source=`echo "$cf_source" |sed -e 's%\.bz2$%%'`
